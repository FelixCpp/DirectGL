...
public:
	...
	explicit Creature(const float spawnX, const float spawnY, const float viewSize) :
		...
		m_MaxLifetime(5.0f),
		m_AttractionFactors {
			{ Target::Food, Math::Random(-3.0f, 3.0f) },
			{ Target::Poison, Math::Random(-3.0f, 3.0f) }
		}
	{
	}
	...
	{
		const auto searchResult = GetNearestFood(food);
	...
	std::optional<SearchResult> GetNearestFood(const std::vector<Math::Float2>& food) const
	{
	...
	void Show() const
	{
		...
		DGL::NoFill();
		DGL::Stroke(DGL::Color(255, 255, 255, 64));
		DGL::StrokeWeight(1.0f);
		DGL::Ellipse(m_Position.X, m_Position.Y, m_ViewSize, m_ViewSize);

		// Compute the direction vector
		const auto foodAttraction = Math::Remap(m_AttractionFactors.at(Target::Food), -5.0f, 5.0f, -50.0f, 50.0f);
		const auto poisonAttraction = Math::Remap(m_AttractionFactors.at(Target::Poison), -5.0f, 5.0f, -50.0f, 50.0f);
...
struct SpikesGame : DGL::Sketch
{
	...
	bool Setup() override
	{
		...
		for (size_t i = 0; i < 10; ++i)
		{
			...
			const float viewSize = std::min(width, height) * 0.25f;

			Creatures.emplace_back(spawnX, spawnY, viewSize);
		}
