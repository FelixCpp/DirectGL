...
IF "%vsversion%" == "vs2010" (
	...
	CALL :VsWhereVsPrereleaseBootstrap "%vsversion%" "18.0"
...
FOR /F "usebackq delims=" %%i in (`!VsWhereCmdLine!`) DO (
	...
	IF EXIST "%%i\VC\Auxiliary\Build\vcvars64.bat" (
		CALL "%%i\VC\Auxiliary\Build\vcvars64.bat" && nmake MSDEV="%PremakeVsVersion%" %PlatformArg% %ConfigArg% -f Bootstrap_Customized.mak windows
		EXIT /B %ERRORLEVEL%
	) ELSE (
		IF EXIST "%%i\VC\Auxiliary\Build\vcvars32.bat" (
			CALL "%%i\VC\Auxiliary\Build\vcvars32.bat" && nmake MSDEV="%PremakeVsVersion%" %PlatformArg% %ConfigArg% -f Bootstrap_Customized.mak windows
			EXIT /B %ERRORLEVEL%
		)
	)
)
...
REM :VsWhereVsPrereleaseBootstrap
:VsWhereVsPrereleaseBootstrap
SET "PremakeVsVersion=%~1"
SET "VsVersion=%~2"
...
SET VsWhereCmdLine="!VsWherePath! -nologo -prerelease -latest -version %VsVersion% -property installationPath"
